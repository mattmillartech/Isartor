...
The `claude_code_benchmark.py` harness provides a **deterministic, phase-based
benchmark** for measuring how Isartor deflects Claude Code traffic when an AI
agent implements a TypeScript todo app end-to-end.
...
The warm-cache deflection rate is high because all 20 prompts are **identical**
between the cold and warm runs — Isartor's L1a exact cache uses a SHA-256 hash
of the full prompt and returns sub-millisecond responses on replay.
...
The `validate_todo_app.py` script checks whether a generated todo app meets
the acceptance criteria from the benchmark task packet.
...
The Claude Code benchmark uses `avg_prompt_tokens = 120` and the Claude Sonnet
input rate of `$0.000003/token` to reflect the longer, more detailed prompts
typical of agentic coding workflows.
